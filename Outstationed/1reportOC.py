...
pd = pandas.read_csv('Kado & Dutse - Sheet1.csv')
...
# print(login_dict)
testimony_list = ['We took out time to pray and plan for the growth of the cell', 'We prayed and planned for the growth'
                  ' of the cell', 'Praying and Planning']
...
for n in range(len(login_dict)):
    ...
    try:
        cell_report = driver.find_element(By.XPATH, '/html/body/div[1]/div[2]/div[3]/div[2]/div[2]/div[2]/div'
                                                    '/div[1]/div'
                                                    '/div/div[4]/a')
        ...
        try:
            ...
            new_attendance = choice(list(range(97, 125)))
            new_first_timers = choice(list(range(14, 18)))
            new_new_converts = choice([new_first_timers - 4, new_first_timers - 6, new_first_timers - 7])
            new_holy_spirit = choice([new_new_converts - 3, new_new_converts - 4, new_new_converts - 5])

            new_midweek = choice(list(range(15, 24)))
            new_church = choice(list(range(20, 28)))
