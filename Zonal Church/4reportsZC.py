...
for n in range(len(login_dict)):
    ...
    for num in range(4):
        ...
        if num == 0:
            ...
            date_of_meeting.send_keys('0411')

            new_attendance = choice(list(range(138, 151)))
            new_first_timers = choice(list(range(15, 32)))
            new_new_converts = choice([new_first_timers - 4, new_first_timers - 5, new_first_timers - 6])
            new_holy_spirit = choice([new_new_converts - 3, new_new_converts - 4, new_new_converts - 5])
            new_midweek = choice(list(range(30, 54)))
            new_church = choice(list(range(88, 122)))
        ...
        # --------Bible Study 1------------------
            ...
            date_of_meeting.send_keys('1111')

            new_attendance = choice(list(range(142, 171)))
            new_first_timers = choice(list(range(15, 32)))
            new_new_converts = choice([new_first_timers - 4, new_first_timers - 5, new_first_timers - 6,
                                       new_first_timers - 7])
            new_holy_spirit = choice([new_new_converts - 3, new_new_converts - 4, new_new_converts - 5])
            new_midweek = choice(list(range(40, 72)))
            new_church = choice(list(range(92, 137)))
        ...
        # --------Bible Study 2------------------
            ...
            date_of_meeting = driver.find_element(By.ID, 'CmisCellLeadersReport_date_of_meeting')
            date_of_meeting.send_keys('1811')

            new_attendance = choice(list(range(161, 192)))
            new_first_timers = choice(list(range(20, 41)))
            new_new_converts = choice([new_first_timers - 4, new_first_timers - 5, new_first_timers - 6,
                                       new_first_timers - 7])
            new_holy_spirit = choice([new_new_converts - 3, new_new_converts - 4, new_new_converts - 5])
            new_midweek = choice(list(range(55, 81)))
            new_church = choice(list(range(110, 151)))
            ...
            new_attendance = choice(list(range(190, 281)))
            new_first_timers = choice(list(range(55, 91)))
            new_new_converts = choice([new_first_timers - 10, new_first_timers - 11, new_first_timers - 12,
                                       new_first_timers - 13, new_first_timers - 14, new_first_timers - 15])
            new_holy_spirit = choice([new_new_converts - 3, new_new_converts - 4, new_new_converts - 5])
            new_midweek = choice(list(range(90, 104)))
            new_church = choice(list(range(130, 192)))
        ...
        try:
            attendance = driver.find_element(By.ID, 'CmisCellLeadersReport_total_no_at_cell_meeting')
            attendance.clear()
            attendance.send_keys(new_attendance)
        except ElementNotInteractableException:
            attendance = driver.find_element(By.ID, 'CmisCellLeadersReport_1_total_no_at_bible_study')
            attendance.clear()
            attendance.send_keys(new_attendance)
